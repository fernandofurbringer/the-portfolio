...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      type: "website",
      locale: locale,
      url: locale === "en" ? "/en/projects" : "/br/projetos",
      title: `${t("title")} | Fernando Cengiz`,
      description: t("description"),
    },
    alternates: {
      canonical: "/projects",
...
export default async function Projects({
  ...
  const projectPreviewData: ProjectPreview[] = projectsData.map((project) => ({
    title: project.title[locale as "en" | "br"],
    src: project.images[0].src,
    alt: project.images[0].alt,
    endpoint: project.endpoint,
    description: project.description[locale as "en" | "br"],
  }));

  return (
    <section className="mb-auto">
      <h1 className="text-fs-700 text-center font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-blue-500 via-purple-700 to-indigo-900 bg-300% animate-flow-gradient my-4">
        {t("title")}
      </h1>
      <div
        aria-hidden
